  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPost(data);
        if (data?.category) {
          const relatedPosts = await base44.entities.BlogPost.filter({ status: 'published', category: data.category }, '-published_date', 4);
          setRelated(relatedPosts.filter((p) => p.id !== id).slice(0, 3));
        }
      } catch (e) { setPost(null); }
      setLoading(false);
    })();
  }, [id]);
